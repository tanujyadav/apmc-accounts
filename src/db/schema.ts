...
export const ledgerHeads = pgTable(
  "ledger_heads",
  {
    id: serial("id").primaryKey(),
  ...
  (table) => [
    uniqueIndex("ledger_heads_code_uidx").on(table.code),
    index("ledger_heads_type_code_idx").on(table.type, table.code),
  ],
);
...
export const bankAccounts = pgTable(
  ...
  (table) => [
    uniqueIndex("bank_accounts_number_uidx").on(table.accountNumber),
...
export const cashbookEntries = pgTable(
  ...
  (table) => [
    index("cashbook_entry_date_idx").on(table.entryDate),
    index("cashbook_head_date_idx").on(table.ledgerHeadId, table.entryDate),
    index("cashbook_bank_date_idx").on(table.bankAccountId, table.entryDate),
    index("cashbook_type_mode_date_idx").on(
      table.entryType,
      table.mode,
      table.entryDate,
    ),
    index("cashbook_party_date_idx").on(table.partyName, table.entryDate),
  ],
);

// Financial-year opening balances shown in the Cashbook register
export const cashbookOpeningBalances = pgTable(
  ...
  (table) => [
    uniqueIndex("cashbook_opening_fy_uidx").on(table.financialYear),
  ],
);
...
export const cheques = pgTable(
  ...
  (table) => [
    index("cheques_date_idx").on(table.chequeDate),
    index("cheques_bank_date_idx").on(table.bankAccountId, table.chequeDate),
    index("cheques_status_direction_idx").on(table.status, table.direction),
    index("cheques_source_bill_idx").on(table.sourceBillId),
  ],
);

// Month-wise Bank Reconciliation Statement manual adjustments
export const brsStatements = pgTable(
  "brs_statements",
  {
  id: serial("id").primaryKey(),
  ...
  (table) => [
    index("brs_month_bank_idx").on(table.statementMonth, table.bankAccountId),
  ],
);
...
export const cashDeposits = pgTable(
  ...
  (table) => [
    uniqueIndex("cash_deposits_slip_uidx").on(table.slipNo),
    index("cash_deposits_date_idx").on(table.depositDate),
    index("cash_deposits_bank_date_idx").on(
      table.bankAccountId,
      table.depositDate,
    ),
  ],
...
export const cashDepositAllocations = pgTable(
  ...
  (table) => [
    index("deposit_alloc_deposit_idx").on(table.cashDepositId),
    index("deposit_alloc_entry_idx").on(table.cashbookEntryId),
    index("deposit_alloc_opening_idx").on(table.cashbookOpeningBalanceId),
  ],
);
...
export const budgets = pgTable(
  "budgets",
  {
    ...
    ledgerHeadId: integer("ledger_head_id")
      .notNull()
      .references(() => ledgerHeads.id),
    mainBudget: numeric("main_budget", { precision: 14, scale: 2 })
      .notNull()
      .default("0"),
    supplementaryBudget: numeric("supplementary_budget", {
      precision: 14,
      scale: 2,
    })
      .notNull()
      .default("0"),
    // Retained for compatibility; always stores Main + Supplementary.
  ...
  (table) => [
    uniqueIndex("budgets_fy_head_uidx").on(
      table.financialYear,
      table.ledgerHeadId,
    ),
  ],
...
// Continuous Bill serial counter for each Financial Year.
...
export const bills = pgTable(
  "bills",
  {
    ...
    ledgerHeadId: integer("ledger_head_id")
      ...
      .references(() => ledgerHeads.id),
    financialYear: text("financial_year").notNull(),
    subVoucherNo: text("sub_voucher_no"),
    authorityDetails: text("authority_details"),
    chequeDetails: text("cheque_details"),
    paymentMode: text("payment_mode").notNull().default("bank"), // cash | bank | cheque
    bankAccountId: integer("bank_account_id").references(
      () => bankAccounts.id,
    ),
    chequeNo: text("cheque_no"), // Net payment cheque
    chequeDate: date("cheque_date"),
    deductionChequeNo: text("deduction_cheque_no"),
    deductionChequeDate: date("deduction_cheque_date"),
    postedCashbookEntryId: integer("posted_cashbook_entry_id").references(
      () => cashbookEntries.id,
      { onDelete: "set null" },
    ),
    amount: numeric("amount", { precision: 14, scale: 2 }).notNull(), // gross total
  ...
  (table) => [
    uniqueIndex("bills_number_uidx").on(table.billNo),
    index("bills_date_idx").on(table.billDate),
    index("bills_fy_status_idx").on(table.financialYear, table.status),
    index("bills_head_idx").on(table.ledgerHeadId),
    index("bills_bank_idx").on(table.bankAccountId),
    index("bills_posted_cashbook_idx").on(table.postedCashbookEntryId),
  ],
);
...
export const shopCollections = pgTable(
  ...
  (table) => [
    index("shop_collections_month_idx").on(table.demandMonth),
    index("shop_collections_shop_month_idx").on(
      table.shopId,
      table.demandMonth,
    ),
    index("shop_collections_status_idx").on(table.status),
  ],
);
...
export const staffMembers = pgTable(
  "staff_members",
  {
    ...
    basicPay: numeric("basic_pay", { precision: 14, scale: 2 })
      .notNull()
      .default("0"),
    status: text("status").notNull().default("active"), // active | inactive
    createdAt: timestamp("created_at").defaultNow().notNull(),
  ...
  (table) => [
    uniqueIndex("staff_employee_code_uidx").on(table.employeeCode),
...
export const staffPayments = pgTable(
  ...
  (table) => [
    index("staff_payments_month_idx").on(table.paymentMonth),
    index("staff_payments_staff_month_idx").on(
      table.staffId,
      table.paymentMonth,
    ),
    index("staff_payments_status_idx").on(table.status),
  ],
);
...
export const tdsReturns = pgTable(
  "tds_returns",
  {
    ...
    tdsAmount: numeric("tds_amount", { precision: 14, scale: 2 })
      .notNull()
      .default("0"),
    interestLateFee: numeric("interest_late_fee", { precision: 14, scale: 2 })
      .notNull()
      .default("0"),
  ...
  (table) => [
    index("tds_fy_quarter_idx").on(table.financialYear, table.quarter),
    index("tds_status_idx").on(table.status),
  ],
);
...
export const revenueTargets = pgTable(
  ...
  (table) => [
    uniqueIndex("revenue_targets_fy_head_uidx").on(
      table.financialYear,
      table.ledgerHeadId,
    ),
  ],
